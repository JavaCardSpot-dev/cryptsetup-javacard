...
public class KeyStorageApplet extends Applet implements ExtendedLength {
    ...
    private static final short STATE_IDLE = (short) 0x4CCC; // int 19660
    private static final short STATE_KEY_ESTABILISHED = (short) 0x5555;// int 21845
    private static final short STATE_AUTHENTICATED = (short) 0x2AAA;// int 10922
    ...
    /**
     * Utility method to write the RSA public key into a buffer. (Copied from
     * HW02 solution)
     *
    ...
    private short processCommand(byte[] buffer, short offset) {
        ...
        switch (command) {
            case CMD_AUTH:
                commandLen = commandAuth(buffer, inDataOffset, commandLen, outDataOffset);
                break;
            case CMD_CHANGEPW:
                commandLen = commandChangePw(buffer, inDataOffset, commandLen, outDataOffset);
                break;
            case CMD_GENKEY:
                commandLen = commandGenKey(buffer, inDataOffset, commandLen, outDataOffset);
                break;
            case CMD_STOREKEY:
                commandLen = commandStoreKey(buffer, inDataOffset, commandLen, outDataOffset);
                break;
            case CMD_LOADKEY:
                commandLen = commandLoadKey(buffer, inDataOffset, commandLen, outDataOffset);
                break;
            case CMD_DELKEY:
                commandLen = commandDelKey(buffer, inDataOffset, commandLen, outDataOffset);
                break;
            case CMD_CLOSE:
                commandLen = commandClose(buffer, inDataOffset, commandLen, outDataOffset);
                break;
            default:
                ISOException.throwIt(ISO7816.SW_INS_NOT_SUPPORTED);
                break;
